...
router.post(
  "/upload",
  upload.fields([{ name: "epub" }, { name: "image" }]),
  async (req, res, next) => {
...
router.get("/convert-to-wav", async (req, res) => {
  ...
  try {
    ...
    // Set the correct content type for WAV file
    res.setHeader("Content-Type", "audio/wav");

    // Send the binary data as is
    res.send(Buffer.from(externalApiResponse.data));
  } catch (error) {
    console.log(error);
    res.status(500).send("Error fetching WAV file");
  }
});
...
router.post("/epubs", async (req, res, next) => {
  try {
    ...
    if (!req.query.keyword) {
      newBooks = await Book.find().sort({ createdAt: -1 }).limit(7).clone();
    }

    res
      .status(200)
      .json({ books: books_filtered, resPerPage, filtered_count, newBooks });
  } catch (error) {
    console.log(error);
